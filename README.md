...
Osobní projekt, který řeší konkrétní potřebu: sledovat srážky v bytě, kde
aktuálně nebydlím, a dostávat o tom okamžité upozornění přímo do aplikace,
kterou už běžně používám (Telegram). Pipeline stahuje počasí z externího
API, ukládá historii do databáze, nabízí ji dál přes vlastní zabezpečené
REST rozhraní, umí vygenerovat AI shrnutí a má jednoduchý webový dashboard.
Vedlejším, ale užitečným efektem je, že projekt zároveň prakticky pokrývá
širokou škálu dovedností z oblasti API integrací, datového managementu,
bezpečnosti a CI/CD - viz mapování níže.
...
## Jak projekt pokrývá profesní dovednosti

Při zpětném pohledu na to, co bylo potřeba postavit, jde vidět, že se to
dobře překrývá s požadavky na běžné pozice typu API/data management
analytik:

| Dovednost | Kde v projektu |
|---|---|
